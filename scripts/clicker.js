  ...
  function purchase_upgrade(id) {
    ...
    if (id == "clicker_upgrade") {
      const new_requiredCost = check_cost(upgrade.clicker_upgrade_purchased, upgrade.cost, clickerCostMultiplier);
      buy_clicker_upgrade(new_requiredCost, 'clicker_upgrade'); // Call function to buy clicker upgrade
    } else if (id == "hp_upgrade") {
      const new_requiredCost = check_cost(upgrade.hp_upgrade_purchased, upgrade.cost, staminaCostMultiplier);
      ...
      const new_requiredCost = check_cost(upgrade.aura_upgrade_purchased, baseCost, auraCostMultiplier);
      buy_aura_upgrade(new_requiredCost, 'aura_upgrade'); // Call function to buy aura upgrade
    } else if (id == "regen_upgrade") {
      const baseCost = upgrade.cost;
      const new_requiredCost = check_cost(upgrade.regen_upgrade_purchased, baseCost, hpRegenCostMultiplier);
  ...
  function check_upgrades() {    
    for (const upgrade in Upgrades) { // Iterates through each upgrade in the Upgrades object
      if (upgrade == "clicker_upgrade") { // Checks if current upgrade is clicker upgrade
        ...
        const new_requiredCost = check_cost(data.clicker_upgrade_purchased, data.cost, clickerCostMultiplier);
        ...
        const new_requiredCost = check_cost(data.hp_upgrade_purchased, data.cost, staminaCostMultiplier);
        ...
        const new_requiredCost = check_cost(data.aura_upgrade_purchased, data.cost, auraCostMultiplier);
        ...
        const new_requiredCost = check_cost(data.regen_upgrade_purchased, data.cost, hpRegenCostMultiplier);
